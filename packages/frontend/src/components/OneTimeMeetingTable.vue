<template>
  <div class="one-time-meeting-table">
    ...
    <DataTable v-else :data="meetings" @row-contextmenu="handleRowContextMenu">
      <el-table-column prop="title" label="名稱" min-width="150">
        <template #default="{ row }">
          <div class="name-cell">
            ...
            <span
              v-if="isToday(row.scheduled_date)"
      ...
      <el-table-column prop="scheduled_date" label="日期" width="110" align="center">
        <template #default="{ row }">
          <span class="date-text">{{ row.scheduled_date }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="耗時" width="80" align="center">
        <template #default="{ row }">
          <span v-if="row.completed_count > 0" class="duration-text">
